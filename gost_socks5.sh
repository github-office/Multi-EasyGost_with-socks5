...
function read_protocol() {
  ...
  echo -e "说明: 只需在中转机设置"
  echo -e "-----------------------------------"
  echo -e "[7] tcp+udp流量转发，通过SOCKS5代理"
  echo -e "说明: 通过指定的SOCKS5代理转发流量到目标地址"
  echo -e "-----------------------------------"
...
function read_d_ip() {
  ...
  elif [ "$flag_a" == "socks5proxy" ]; then
    echo -e "------------------------------------------------------------------"
    echo -e "请输入SOCKS5代理地址（格式：IP:端口，例如 127.0.0.1:20170）:"
    read -p "请输入: " socks5_proxy
    echo -e "是否需要SOCKS5代理认证？[y/n]:"
    read -p "请输入: " socks5_auth
    if [[ "$socks5_auth" == [Yy] ]]; then
      echo -e "请输入SOCKS5用户名:"
      read -p "请输入: " socks5_user
      echo -e "请输入SOCKS5密码:"
      read -p "请输入: " socks5_pass
      flag_c="socks5://$socks5_user:$socks5_pass@$socks5_proxy"
    else
      flag_c="socks5://$socks5_proxy"
    fi
  elif [[ "$flag_a" == "decrypttls_socks5" || "$flag_a" == "decryptws_socks5" || "$flag_a" == "decryptwss_socks5" ]]; then
    echo -e "------------------------------------------------------------------"
    echo -e "请输入SOCKS5代理地址（格式：IP:端口，例如 127.0.0.1:20170）:"
    read -p "请输入: " socks5_proxy
    echo -e "是否需要SOCKS5代理认证？[y/n]:"
...
function read_d_port() {
  if [ "$flag_a" == "ss" ]; then
    ...
    echo -e "------------------------------------------------------------------"
    echo -e "请问你要将本机从${flag_b}接收到的流量转发向哪个IP或域名?"
    read -p "请输入: " target_ip
    echo -e "请问你要将本机从${flag_b}接收到的流量转发向${target_ip}的哪个端口?"
    read -p "请输入: " target_port
    flag_d="$target_ip:$target_port"
  elif [[ "$flag_a" == "decrypttls_socks5" || "$flag_a" == "decryptws_socks5" || "$flag_a" == "decryptwss_socks5" ]]; then
    echo -e "------------------------------------------------------------------"
    echo -e "请问你要将解密后的流量通过SOCKS5代理转发向哪个IP或域名?"
    read -p "请输入: " target_ip
    echo -e "请问你要将解密后的流量转发向${target_ip}的哪个端口?"
    read -p "请输入: " target_port
    flag_d="$target_ip:$target_port"
  else
    echo -e "------------------------------------------------------------------"
...
function writerawconf() {
  if [[ "$flag_a" == "socks5proxy" || "$flag_a" == "decrypttls_socks5" || "$flag_a" == "decryptws_socks5" || "$flag_a" == "decryptwss_socks5" ]]; then
...
function decrypt() {
  echo -e "------------------------------------------------------------------"
  echo -e "请问您要解密的gost流量类型: "
  echo -e "-----------------------------------"
  echo -e "[1] relay+tls"
  echo -e "[2] relay+ws"
  echo -e "[3] relay+wss"
  echo -e "[4] relay+tls with SOCKS5 proxy"
  echo -e "[5] relay+ws with SOCKS5 proxy"
  echo -e "[6] relay+wss with SOCKS5 proxy"
  echo -e "-----------------------------------"
  ...
  if [ "$numdecrypt" == "1" ]; then
    ...
    flag_a="decrypttls_socks5"
  elif [ "$numdecrypt" == "5" ]; then
    flag_a="decryptws_socks5"
  elif [ "$numdecrypt" == "6" ]; then
    flag_a="decryptwss_socks5"
  else
    echo "type error, please try again"
    exit
  fi
...
function method() {
  if [ $i -eq 1 ]; then
    ...
    elif [ "$is_encrypt" == "socks5proxy" ]; then
      echo "        \"tcp://:$s_port/$d_port\",
      \"udp://:$s_port/$d_port\"
    ],
    \"ChainNodes\": [
        \"$d_ip\"" >>$gost_conf_path
    ...
    elif [ "$is_encrypt" == "decrypttls_socks5" ]; then
      if [ -d "$HOME/gost_cert" ]; then
        echo "        \"relay+tls://:$s_port/$d_port?cert=/root/gost_cert/cert.pem&key=/root/gost_cert/key.pem\",
        \"udp://:$s_port/$d_port\"
      ],
      \"ChainNodes\": [
          \"$d_ip\"" >>$gost_conf_path
      else
        echo "        \"relay+tls://:$s_port/$d_port\",
        \"udp://:$s_port/$d_port\"
      ],
      \"ChainNodes\": [
          \"$d_ip\"" >>$gost_conf_path
      fi
    elif [ "$is_encrypt" == "decryptws_socks5" ]; then
      echo "        \"relay+ws://:$s_port/$d_port\",
      \"udp://:$s_port/$d_port\"
    ],
    \"ChainNodes\": [
        \"$d_ip\"" >>$gost_conf_path
    elif [ "$is_encrypt" == "decryptwss_socks5" ]; then
      if [ -d "$HOME/gost_cert" ]; then
        echo "        \"relay+wss://:$s_port/$d_port?cert=/root/gost_cert/cert.pem&key=/root/gost_cert/key.pem\",
        \"udp://:$s_port/$d_port\"
      ],
      \"ChainNodes\": [
          \"$d_ip\"" >>$gost_conf_path
      else
        echo "        \"relay+wss://:$s_port/$d_port\",
        \"udp://:$s_port/$d_port\"
      ],
      \"ChainNodes\": [
          \"$d_ip\"" >>$gost_conf_path
